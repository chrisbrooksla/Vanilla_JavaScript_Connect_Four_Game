document.addEventListener('DOMContentLoaded', () => {
      // selects anything that is a div, or has classname of "grid"...
      const squares = document.querySelectorAll('.grid div')

      // selects the <h3> with ID of "result"
      const result = document.querySelector('#result')

      // selects the span with the ID "current-player"
      const displayCurrentPlayer = document.querySelector('#current-player')

      let currentPlayer = 1

      // this loops over each square...
      for (let i = 0, len = squares.length; i < len; i++)

      (function(index){
            //add an onclick to each square in your grid
                squares[i].onclick = function(){
                    //if the square below your current square has class name of "taken", you can go on top of it (claim it as your own)
            ...
            function checkBoard() {
                  // make a const that shows all the winning arrays...
                  const winningArrays = [
                        [0, 1, 2, 3],
                        [41, 40, 39, 38],
                        [7, 8, 9, 10],
